    ...
    for (const seed of readySeeds) {
      ...
      try {
        ...
        if (opts?.runtimeMode === "test") {
          log(`[foreman] Skipping workflow setup/install for ${seedForDispatch.id} in test runtime`);
        ...
        if (workspaceWasCreated && projectHooks?.afterCreate) {
          ...
          try {
            ...
            throw new Error(`afterCreate hook failed for ${seedForDispatch.id}: ${hookMsg}`);
          }
        }
